...
class ParameterSelection:
    ...
    def __init__(
        self, df, videofilepath, bgs_type, startat, nframes, roi, nroi, path=None
    ):
        self.df = df
    ...
    def get_videofragment(self):
        ...
        """
        return self.vfs.iter_frames(startat=self.startat, nframes=self.nframes)
    ...
    def get_parameters(self, selection="multi_only"):
        ...
        """
        paramkeys = set(self.df.columns) - set(["framenumber", "pixel", "coords"])
        ...
        if selection == "all":
            ...
            return {
                k: all_parameters[k] for k in paramkeys if len(all_parameters[k]) > 1
            }
        else:
            raise Exception(
                f"'{selection}' is not recognized. Please "
    ...
    def plot_parameters(self, rows, cols, default_values):
        ...
        other_values = {
            key: default_values[key] for key in default_values.keys() - [rows, cols]
        }
    ...
    def draw_multiple_circles(self, all_settings, radius=60, thickness=2):
        ...
        for i, settings in enumerate(product_dict(**all_settings)):
            ...
            frames = frames.draw_circles(
                iterdata, radius=radius, color=self.colors[i][1], thickness=thickness
            )
    ...
    def _check_multi_only(self, inputdict):
        ...
        if multi_only.keys() != inputdict.keys():
            raise Exception(
                "Make sure the input dictionary contains all keys"
    ...
    def _check_stats_is_not_count(self):
        if self.df.coords.unique()[0] == "count":
            raise Exception("This function cannot be executed with count statistics.")
...
def apply_all_parameters(
    ...
    list_with_dfs = []
    all_combinations = product_dict(
        **all_settings["bgs_params"], **all_settings["processing"]
    )

    for settings in all_combinations:
        frames = md.apply_settings(vfs, settings, startat, nframes, roi, nroi, bgs_type)
        coordinates = frames.find_nonzero()

        # calculate mean or count statistics of coordinates
        if use_stats == "mean":
            coordstats = np.array(
                [c.mean(0) if c.size > 0 else (np.nan, np.nan) for c in coordinates]
            )
            settings["coords"] = ["x", "y"]
            ...
            raise Exception("Make sure use_stats has 'mean' or 'count' as input value")
        ...
        if reportprogress:
            ...
            if n % reportprogress == 0:
                ...
                print(
                    f"{n} combinations of settings applied in"
    ...
    df = (
        df.stack(df.columns.names, future_stack=True)
        .reset_index()  # stack all column levels
        .rename({0: "pixel"}, axis=1)
    )

    return ParameterSelection(
        df, vfs.filepath, str(bgs_type), startat, nframes, roi, nroi
    )
