...
a = a.reshape((497, 2), order='C')

Python with Numpy (memmap):
---------------------------
import numpy as np
a = np.memmap('arrayvalues.bin', dtype='<f8', shape=(497, 2), order='C')

R:
--
fileid = file("arrayvalues.bin", "rb")
a = readBin(con=fileid, what=numeric(), n=994, size=8, signed=TRUE, endian="little")
a = array(data=a, dim=c(2, 497), dimnames=NULL)
close(fileid)

Matlab/Octave:
--------------
fileid = fopen('arrayvalues.bin');
a = fread(fileid, [2, 497], '*float64', 'ieee-le');
fclose(fileid);

Julia (version < 1.0):
----------------------
fileid = open("arrayvalues.bin","r");
a = map(ltoh, read(fileid, Float64, (2, 497)));
close(fileid);

Julia (version >= 1.0):
-----------------------
fileid = open("arrayvalues.bin","r");
a = map(ltoh, read!(fileid, Array{Float64}(undef, 2, 497)));
close(fileid);

IDL/GDL:
--------
a = read_binary("arrayvalues.bin", data_type=5, data_dims=[2, 497], endian="little")

Mathematica:
------------
a = BinaryReadList["arrayvalues.bin", "Real64", ByteOrdering -> -1];
a = ArrayReshape[a, {497, 2}];

Maple:
------
a := FileTools[Binary][Read]("arrayvalues.bin", float[8], byteorder=little, output=Array);
a := ArrayTools[Reshape](a, [2, 497]);
