...
default_settings = {
    "processing": {
        "color": False,  # booleans only
        "blur": 0,  # use '0' for no blur
        "morphologyex": True,  # booleans only
        "resizebyfactor": 1,
    }
}  # use '1' for no change in size


def batch_detect_movement(
    vfs_list,
    settings=None,
    startat=None,
    nframes=None,
...
def detect_movement(
    ...
    cd = frames.save_nonzero(
        Path(movementpath) / "coords.darr",
        metadata=metadata,
        ignore_firstnframes=ignore_firstnframes,
        overwrite=overwrite,
    )
    cc = darr.asarray(
        Path(movementpath) / "coordscount.darr",
        cd.get_coordcount(),
        metadata=metadata,
        overwrite=overwrite,
    )
    cm = darr.asarray(
        Path(movementpath) / "coordsmean.darr",
        cd.get_coordmean(),
        metadata=metadata,
        overwrite=overwrite,
    )
    ...
    if resultvideo:
        create_movementvideo(
            ...
            startat=startat,
            nframes=nframes,
            videofilepath=Path(movementpath) / "movementvideo.mp4",
            overwrite=overwrite,
        )
    return cd, cc, cm
...
def apply_settings(
    ...
    """
    frames = vfs.iter_frames(startat=startat, nframes=nframes, color=settings["color"])
    ...
    frames = frames.apply_backgroundsegmenter(bgs, learningRate=-1, roi=roi, nroi=nroi)
...
def create_movementvideo(
    ...
    frames = coords.iter_frames(nchannels=3, value=(0, 0, 255)).add_weighted(
        0.8, videoframes, 0.7
    )
