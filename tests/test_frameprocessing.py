...
colorlist = [
    framecolor(height=480, width=640, color=(0, 0, 0)),
    framecolor(height=480, width=640, color=(1, 1, 1)),
]

graylist = [
    framegray(height=480, width=640, value=0),
...
class TestFrames(unittest.TestCase):
    ...
    def test_calcmeanframe(self):
        frames = Frames(
            [
                framecolor(height=480, width=640, color=(10, 10, 10)),
...
class TestFindNonZero(unittest.TestCase):
    ...
    def test_colorframe(self):
        ...
        assert_array_equal(next(idx), np.array([[1, 1], [2, 1], [3, 1]], dtype="int32"))

    def test_saveascoords(self):
        frames = Frames([self.fcolor, self.fcolor])
        tempdirname = tempfile.mkdtemp()
        coordsarray = frames.save_nonzero(
            filepath=tempdirname, metadata={"avgframerate": 5}
        )
